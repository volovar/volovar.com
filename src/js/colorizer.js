document.addEventListener("DOMContentLoaded", function (event) {
  (function () {
    ...
    //for (var i = 0; i < mark.children.length; i++) { mark.children[i].style.fill =  }
    ...
    function changeLogoColor() {
      var mark = document.querySelector("#js-v-mark");

      for (var i = 0; i < mark.children.length; i++) {
        setDelay(mark.children[i], i, shadeColor(colorSet[colorCount], (i * -0.11)));
      }
    }
    ...
    function darkenColors() {
      for (var i = 0; i < (elements.length); i++) {
        ...
        setDelay(elements[i], i, blockColor);
      }
    ...
    function setDelay(element, time, color) {
