...
const ajaxRequest = new (function () {
    ...
    const sTargetId = "ajax-content", sViewKey = "view_as", sAjaxClass = "ajax-nav",

    /* not customizable constants */
    const rSearch = /\?.*$/;
    const rHost = /^[^\?]*\?*&*/;
    const rView = new RegExp("&" + sViewKey + "\\=[^&]*|&*$", "i");
    const rEndQstMark = /\?$/;
    const oLoadingBox = document.createElement("div");
    const oCover = document.createElement("div");
    const oLoadingImg = new Image();
    const oPageInfo = {
      title: null,
      url: location.href
    };
    const oHTTPStatus = /* http://www.iana.org/assignments/http-status-codes/http-status-codes.xml */ {
            100: "Continue",
